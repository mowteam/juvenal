...
- Dependency-stranded target (`dependency_stranded_targets` in the delta) — the opposite case, and the one exception to the rule above. The target was never dispatched: its `depends_on_claim_ids` named a claim that ended up rejected, so the gate can never open. Nothing was learned about the work itself and nothing is known to be infeasible. If the work is still worth doing, **re-enqueue it now with `depends_on_claim_ids: []`**, carrying the original instructions across and inlining any mechanism it needs from the rejected claim. If it is not worth doing, say so in `mental_model_summary` — but do not leave it stranded by default, because that silently discards work you chose to schedule.
